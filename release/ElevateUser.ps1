...
#    Write-Log -Message "oServerName = $oserverName oServerDomainNameBiosName = $oServerDomainNetBiosName oServerDNSDomain = $oServerDNSDomain" -Severity Debug
    ...
    if ($config.EnableDelegation){
        ...
        if (!(Get-UserElevationStatus -ServerName $oServer.DNSHostName -UserName $authorizationUserName -DelegationConfig $config.DelegationConfigPath -AllowManagedByAttribute $config.UseManagedByforDelegation)){
